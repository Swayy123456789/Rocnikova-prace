  ...
  return (
    ...
    >
      <div className="!p-4">
        ...
        {products.length === 0 ? (
          <p className="!mx-auto text-sm text-gray-500">Your cart is empty.</p>
        ) : (
          <ul className="space-y-2 !max-h-60">
            {products.map((product) => (
              <li
                key={product.id}
                className="text-sm text-gray-700 flex justify-between !py-1 !px-2 hover:bg-gray-200 rounded-md transition">
                <span className="!max-w-[60%] text-black">{product.name} × {product.quantity}</span>
                <span className="font-bold text-white">{(product.price * product.quantity)} $</span>
              </li>
            ))}
          </ul>
        )}
      ...
      {products.length > 0 && (
        ...
        <div className="!px-4 !py-3 border-t text-gray-800">
